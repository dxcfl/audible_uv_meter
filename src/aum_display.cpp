/*
  ...
  Display for the audible UV meter application ...
...
public:
  ...
  static void screen()
  {
    ...
    lv_label_set_text(uv_label, "UV (mW/m^2)");
  ...
  {
    lv_tick_handler();
    lv_task_handler(); /* let the Display do its work */
    delay(5);
    lv_refr_now(NULL);
  }
   ...
   */
  static void update_labels(float uv, float uv_index)
  {
    lv_label_set_text_fmt(uv_value_label, "%0.1f", uv);
    lv_label_set_text_fmt(uvi_value_label, "%0.1f", uv_index);
  }

  /* Updates the line meter and it's label  ...
   */
  static void update_meter(float uv_index)
  {
